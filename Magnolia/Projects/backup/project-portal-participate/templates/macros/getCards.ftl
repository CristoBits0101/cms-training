...
[#macro getCards targetWorkspace=""]
    ...
    [#if targetWorkspace?has_content && targetWorkspace == "news"]
        ...
        [#if content.newsNumber?has_content && content.newsNumber > 0]

            [#-- If newsOrderReversed is null, the conditional won't execute, skipping the exception error --]
            [#if newsOrderReversed?? && (newsOrderReversed?size == content.newsNumber) || (newsOrderReversed?size > content.newsNumber) && (newsOrderReversed?size > 0)]
        ...
        [#else]
            ...
            [#-- We check that there is stored content --]
            [#if newsOrderReversed??]

                [#-- List the collection and store a node type in childnode at each iteration --]
                [#list newsOrderReversed[0..(content.newsNumber!1)-1] as childNode]
